...
const UPLOAD_DIR = path.join(UPLOADS_BASE_DIR, 'blogs');
fs.mkdirSync(UPLOAD_DIR, { recursive: true });

function getUploadFilePath(imagePath) {
...
async function updateBlog(req, res, next) {
  try {
    ...
    if (mainImage) {
      ...
      if (image_path) {
        const old = getUploadFilePath(image_path);
        try { if (old && fs.existsSync(old)) fs.unlinkSync(old); } catch (e) {}
      }
...
async function deleteBlog(req, res, next) {
  try {
    ...
    if (row.image_path) {
      const file = getUploadFilePath(row.image_path);
